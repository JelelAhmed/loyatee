    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 backdrop-blur-sm">
      <div className="bg-[var(--card-background-color)]/100 p-6 rounded-xl w-[90%] max-w-md border border-white/20 shadow-2xl space-y-5">
        ...
        <div className="flex justify-end gap-3 mt-2">
          <button
            onClick={onCancel}
            className="px-4 py-2 rounded-lg border border-white/20 text-sm text-[var(--text-secondary)] hover:bg-white/5 transition-colors"
          >
            {cancelText}
          </button>
          <button
            ...
            disabled={requireEmailInput && !inputEmail.trim()}
          >
            {confirmText}
          </button>
        </div>
