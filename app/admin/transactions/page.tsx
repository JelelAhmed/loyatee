...
//     // Fetch users for email/phone mapping (unique IDs to avoid duplicates)
//     const userIds = [
//       ...new Set([
...
//     console.log("Fetched user IDs:", userIds.length);
//     console.log("Fetched users data:", usersData?.length || 0);
...
//         payment_reference: f.payment_reference ?? "-", // Use payment_reference here
...
export default function TransactionsPage() {
  ...
  const fetchTransactions = async () => {
    ...
    // Fetch ALL users (like dashboard) for email/phone mapping
    const { data: usersData, error: usersError } = await supabase
      .from("users")
      .select("id, email, phone") // Changed to 'phone'
      .order("created_at", { ascending: false }); // No .in() filter—full fetch for mapping

    // Debug: Log to check RLS/fetch issues
    console.log("Fetched users data length:", usersData?.length || 0);
    console.log("Users fetch error:", usersError?.message);
    if (usersData && usersData.length > 0) {
      console.log(
        "Sample users:",
        usersData.slice(0, 2).map((u) => ({ id: u.id, email: u.email }))
      );
    }

    if (usersError) {
      console.error("Users fetch failed (likely RLS):", usersError.message);
    }
    ...
    const transactionMapped = (txData ?? []).map((t: any) => {
      ...
      return {
        ...
        dispute_note: t.dispute_note,
        dispute_type: t.dispute_type,
        phone_number: t.phone_number ?? user?.phone ?? "Unknown phone", // Changed to user?.phone
        created_at: t.created_at,
        payment_method: t.payment_method ?? "-",
        payment_reference: t.payment_reference ?? "-", // For completed fundings
        error_message: t.error_message ?? null,
        vendor_response:
          typeof t.vendor_response === "string"
            ? JSON.parse(t.vendor_response || "{}")
    ...
    const pendingMapped = (fundingsData ?? []).map((f: any) => {
      ...
      return {
        ...
        dispute_note: null,
        dispute_type: null,
        phone_number: user?.phone ?? "-", // Changed to user?.phone
        created_at: f.created_at,
        payment_method: f.payment_method ?? "-",
        payment_reference: f.payment_reference ?? "-", // Use payment_reference for pending/failed
        error_message: f.error_message ?? null,
        vendor_response:
          typeof f.vendor_response === "string"
            ? JSON.parse(f.vendor_response || "{}")
          ...
          <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center w-full sm:w-auto">
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <select
                ...
                className="flex-1 sm:flex-none min-w-[120px] bg-[#1f1f2e] border border-[#44475a] rounded-lg px-3 py-2 text-sm text-[#f8f8f2] focus:ring-2 focus:ring-[var(--primary-color)]"
              >
                <option value="data_purchase">Data Purchases</option>
                <option value="wallet_funding">Wallet Fundings</option>
              </select>
            ...
            <input
              ...
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search user, amount, status..."
              className="w-full sm:w-auto min-w-[200px] bg-[#1f1f2e] border border-[#44475a] rounded-lg px-3 py-2 text-sm text-[#f8f8f2] placeholder-[#6272a4] focus:ring-2 focus:ring-[var(--primary-color)]"
            />
          </div>
        </div>
