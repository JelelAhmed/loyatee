...
export async function POST(request: Request) {
  ...
  if (event === "charge.success") {
    ...
    try {
      ...
      if (!funding) {
        console.log("⚠️ Already processed or invalid reference:", reference);
      ...
      if (txError && txError.code !== "23505") throw new Error(txError.message);

      // Step 3: Increment balance if transaction was new
      if (!txError) {
